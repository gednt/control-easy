...
3. **Lifecycle Stop Hook:**
   The Stop hook in `.agents/hooks.json` automatically blocks the agent from finishing if code modifications exist and `scripts/verify-ci-local.sh` has not passed.
