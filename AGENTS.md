...
- **Mandatory Local CI Verification Gate.** Agents MUST NOT consider any modification, task, or bugfix "done" until all jobs in the CI pipeline pass locally via `scripts/verify-ci-local.sh` (or `make verify-ci`, or `scripts/verify-ci-local.ps1` on Windows). Claiming completion, checking off tasks, or pushing code without running and passing the local CI suite on the final modified state is a strict constitutional violation. The gate validates: (1) C# code format verification (`dotnet format --verify-no-changes`), (2) solution build in Release mode, (3) unit & architecture tests, (4) Testcontainers MySQL integration tests, (5) Angular production build, and (6) in-process OpenAPI swagger generation and client drift check (`git diff --exit-code -- src/app/api`). Automated agent Stop hooks in `.agents/hooks.json`, `.agent/hooks.json`, `.claude/settings.local.json`, `.codex/hooks.json`, and `.cursor/hooks.json` enforce this gate before allowing an agent loop to conclude.
...
- **Post-task verification (Mandatory Local CI Gate):** After every implementation task or code modification, the agent MUST run `scripts/verify-ci-local.sh` (or `make verify-ci`). No task or modification is done until all local CI jobs pass. An agent that concludes work without passing this gate is in violation of project policy. Stop hooks automatically block agents from completing if the local CI gate has not passed.
- **Pre-commit adversarial review:** No agent may `git commit` until (1) build + tests are green (including the local CI gate) and (2) an adversarial review subagent returns zero CRITICAL/HIGH findings. See existing AGENTS.md archive for full gate procedure.
