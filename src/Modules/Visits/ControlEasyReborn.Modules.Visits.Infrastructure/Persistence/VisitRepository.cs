...
public sealed class VisitRepository : IVisitRepository
{
    ...
    {
        var db = _factory.Create(_ctx);
        var whereParts = new List<string> { "ApartmentId IS NOT NULL" };
        var parameters = new List<object>();
        if (!string.IsNullOrWhiteSpace(status))
        {
            whereParts.Add("Status = @param" + parameters.Count);
            parameters.Add((int)Enum.Parse(typeof(VisitStatus), status, true));
        }
    ...
    public async Task<IReadOnlyList<Visit>> ListOpenAsync(Guid tenantId, CancellationToken ct)
    {
        ...
        var rows = await db.SelectAsync(
            fields: Fields,
            table: TableName,
            whereClause: "ApartmentId IS NOT NULL AND Status IN (0, 1)",
    ...
    public async Task AddAsync(Visit visit, CancellationToken ct)
    {
        ...
        await db.InsertAsync(
            new[] { "Id", "TenantId", "VisitorName", "VisitorDocument", "VisitorPhone", "ApartmentId", "DestinationBlock", "DestinationUnit", "Purpose", "Status", "AttendantProfileId", "GatehouseId", "CheckedInAtUtc", "CheckedOutAtUtc", "CreatedAtUtc", "UpdatedAtUtc", "tenant_id" },
            TableName,
            new object?[] { visit.Id, visit.TenantId, visit.VisitorName, visit.VisitorDocument, (object?)visit.VisitorPhone ?? DBNull.Value, visit.ApartmentId, visit.DestinationBlock, visit.DestinationUnit, (object?)visit.Purpose ?? DBNull.Value, (int)visit.Status, (object?)visit.AttendantProfileId ?? DBNull.Value, (object?)visit.GatehouseId ?? DBNull.Value, (object?)visit.CheckedInAtUtc ?? DBNull.Value, (object?)visit.CheckedOutAtUtc ?? DBNull.Value, visit.CreatedAtUtc, (object?)visit.UpdatedAtUtc ?? DBNull.Value, visit.TenantId },
            primaryKeyName: "Id",
            autoIncrement: false,
            ct: ct);
    }
    ...
    private static Visit? MapRow(DataRow r)
    {
        ...
        if (string.IsNullOrEmpty(apartmentIdStr)) return null;
        var destinationBlock = r["DestinationBlock"]?.ToString();
        var destinationUnit = r["DestinationUnit"]?.ToString();
        if (string.IsNullOrEmpty(destinationBlock) || string.IsNullOrEmpty(destinationUnit)) return null;
        ...
        return new Visit(
            ...
            apartmentId: Guid.Parse(apartmentIdStr),
